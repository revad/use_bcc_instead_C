UseBccInstead.UseBccInstead =
{
  ...
  //window.addEventListener("compose-send-message", UseBccInstead.eventHandler, true);
  },
...
//},
  ...
  confirm: function()
  {
    ...
    var forceNoSend = UseBccInstead.UseBccInsteadUtil.getBoolPref("extensions.usebccinstead.forceNoSend", false);

    if(forceNoSend)
    {
      title += UseBccInstead.UseBccInsteadUtil.getLocalizedString("confirm.NoSendTitle");
      message = UseBccInstead.UseBccInsteadUtil.getLocalizedString("confirm.NoSendMessage");
      this.prompts.alert(window, title, message);
      return false;
    }

    title += UseBccInstead.UseBccInsteadUtil.getLocalizedString("confirm.Title");
    message = UseBccInstead.UseBccInsteadUtil.getLocalizedString("confirm.Message");

    var soundSource = null;

    switch(UseBccInstead.UseBccInsteadUtil.getOsType())
    {
      case "WINNT":
      {
        soundSource = "chrome://usebccinstead/content/UseBccAlert.wav";
        break;
      }
    ...
    var play = UseBccInstead.UseBccInsteadUtil.getBoolPref("extensions.usebccinstead.playSound", false);
    ...
    var lastCheckedState = UseBccInstead.UseBccInsteadUtil.getBoolPref("extensions.usebccinstead.lastCheckedState", false);

    var checkedState = {value: lastCheckedState};
    var result = this.prompts.confirmCheck(window, title, message, UseBccInstead.UseBccInsteadUtil.getLocalizedString("confirm.checkLabel"), checkedState);
    if(!result)
    {
    ...
    else
    {
      ...
      {
        UseBccInstead.UseBccInsteadUtil.setBoolPref("extensions.usebccinstead.lastCheckedState", true);
        return this.sendWithConversion;
      }
      else
      {
        UseBccInstead.UseBccInsteadUtil.setBoolPref("extensions.usebccinstead.lastCheckedState", false);
  ...
  splitParts: function(recipient)
  {
    ...
    if(j > -1)
    {
      ...
      if((k > -1) && (k > j))
      {
        ...
        result[0] = UseBccInstead.UseBccInsteadUtil.trim(recipient.substr(0, (j-1)));
        result[0] = this.cleanString(result[0]);
        result[1] = UseBccInstead.UseBccInsteadUtil.trim(recipient.substr((j+1), (k-j-1)));
    ...
    else
    {
      ...
      result[0] = UseBccInstead.UseBccInsteadUtil.trim(recipient);
      result[0] = this.cleanString(result[0]);
    }
  ...
  isMailingList: function(recipient)
  {
    ...
    while(addressBooks.hasMoreElements())
    {
      ...
      if(addrbook instanceof Components.interfaces.nsIAbDirectory)
      {
        ...
        var count = (UseBccInstead.UseBccInsteadUtil.isTB2()) ? addrbook.addressLists.Count() : addrbook.addressLists.length;
  ...
  check: function()
  {
    ...
    var forceBcc = UseBccInstead.UseBccInsteadUtil.getBoolPref("extensions.usebccinstead.forceBcc", false);
    ...
    if(UseBccInstead.UseBccInsteadUtil.isTB24())
    {
    ...
    for(var i = 0; i < recipientCount; i++)
    {
      ...
      if(recipientType)
      {
        ...
        if(recipientAddr)
        {
          recipient = UseBccInstead.UseBccInsteadUtil.trim(recipientAddr.value);
        }
        ...
        else
        {
          ...
          {
            if("" != UseBccInstead.UseBccInsteadUtil.trim(recipientArray[j]))
            {
              recipientsOnLine++;
            }
          }
    ...
    var configuredCount = UseBccInstead.UseBccInsteadUtil.getIntPref("extensions.usebccinstead.nonBccCount", 10);
...
UseBccInstead.UseBccInstead.init();

SendMessage = function()
{
  UseBccInstead.UseBccInstead.invokeOriginalSend(UseBccInstead.UseBccInstead.originalSendMessage);
}

SendMessageWithCheck = function()
{
  UseBccInstead.UseBccInstead.invokeOriginalSend(UseBccInstead.UseBccInstead.originalSendMessageWithCheck);
}

SendMessageLater = function()
{
  UseBccInstead.UseBccInstead.invokeOriginalSend(UseBccInstead.UseBccInstead.originalSendMessageLater);
}
